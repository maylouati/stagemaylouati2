...
class CreateClientdnlinesTable extends Migration {
	...
	public function up()
	{
		Schema::create('clientdnlines', function(Blueprint $table)
		{
			$table->uuid('id')->primary('ClientDnLines_pkey');
			...
			$table->uuid('clientdn_id')->index('ClientDnLines_IX_ClientDn_id');
			$table->uuid('clientorderline_id')->index('ClientDnLines_IX_ClientOrderLine_id');
			$table->uuid('item_id')->index('ClientDnLines_IX_Item_id');
			$table->uuid('unit_id')->index('ClientDnLines_IX_Unit_id');
		});
	}
