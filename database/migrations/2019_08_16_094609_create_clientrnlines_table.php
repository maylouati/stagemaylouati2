...
class CreateClientrnlinesTable extends Migration {
	...
	public function up()
	{
		Schema::create('clientrnlines', function(Blueprint $table)
		{
			$table->uuid('id')->primary('ClientRnLines_pkey');
			...
			$table->uuid('clientdnline_id')->nullable()->index('ClientRnLines_IX_ClientDnLine_id');
			$table->uuid('clientrn_id')->index('ClientRnLines_IX_ClientRn_id');
			$table->uuid('item_id')->nullable()->index('ClientRnLines_IX_Item_id');
			$table->uuid('unit_id')->nullable()->index('ClientRnLines_IX_Unit_id');
		});
	}
